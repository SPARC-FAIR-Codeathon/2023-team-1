...
WHERE {
  ?element owl:equivalentClass ?value .
}
"""

results = graph.query(query)

def retrieve_bnode_properties(bnode, additional_info):
    ...
    for s, p, o in triples_with_bnode_as_subject:
        ...
        if isinstance(o, BNode):
            object_value = retrieve_bnode_properties(o, additional_info)  # Recursive call for nested BNode
        else:
            object_value = o.toPython() if isinstance(o, (int, float)) else str(o)

        if 'onProperty' in object_value and object_value['onProperty'] == 'http://uri.interlex.org/tgbugs/uris/readable/hasInstanceInTaxon':
            additional_info['hasInstanceInTaxon'] = object_value['someValuesFrom']

        if 'onProperty' in object_value and object_value[
            'onProperty'] == 'http://uri.interlex.org/tgbugs/uris/readable/hasBiologicalSex':
            additional_info['hasBiologicalSex'] = object_value['someValuesFrom']

        bnode_representation[predicate] = object_value
    return bnode_representation

neuron_properties = dict()

# Set the group properties for each element
for row in results:
    element = row['element']
    bnode = row['value']
    ...
    bnode_representation = retrieve_bnode_properties(bnode, additional_info)

    neuron = str(element).replace("http://uri.interlex.org/tgbugs/uris/readable/sparc-nlp/", "")
    neuron_properties[neuron] = additional_info
...
GROUP_COLUMNS = ['id', 'name', 'lyphs', "links", "nodes", "hasBiologicalSex", "hasInstanceInTaxon"]
...
'''
cql_equivalence_class = '''
    MATCH (src {uri:$uri})-[:equivalentClass]->(d)-[:intersectionOf|first|rest*..]->(dst) RETURN dst
'''
...
with driver.session() as session:
    ...
    for neuron in neurons:
        ...
        res2 = session.run(cql_equivalence_class, uri=uri)
        attributes = [record for record in res2.data()]
        ...
        if len(chains) > 1:
            ...
            sex = neuron_properties[baseID].get('hasBiologicalSex', 'N/A')
            species = neuron_properties[baseID].get('hasInstanceInTaxon', 'N/A')

            # Create neuron group
            df_groups.loc[len(df_groups.index)] = ["g_neuron_"+baseID, "Group "+baseID, "", lnk_set_str, node_set_str, sex, species]
        else:
